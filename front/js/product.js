...
const apiProductId = fetch("http://localhost:3000/api/products/" + `${monId}`)
  ...
  .then((data) => {
    ...
    button.addEventListener("click", () => {
      ...
      var commande = JSON.parse(localStorage.getItem("product"));
      //si newColor = true, verifie newQuantity
      if (newColor) {
        //si newQuantity plus grand ou égale à 1, verifie newQuantity <= 100
        if (newQuantity >= 1) {
          //si newQuantity plus petit ou égale à 100, verifie commande
          if (newQuantity <= 100) {
            //si commande déja présente, collecte d'information et vérifie s'il y a des doublons
            if (commande) {
              //cherche tous les id dans le tableau d'objet commande
              let listeId = commande.map((el) => el.id);
              //retourne l'index de l'objet où se trouve l'id du produit à ajouter au panier
              let indexDoublonId = listeId.indexOf(id);
              //retourne true ou false celon si l'id du produit à ajouter est déja présent ou non dans le tableau d'objet commande
              let booleanDoublonId = listeId.includes(id);
              //cherche tous les color dans le tableau d'objet commande
              let listColors = commande.map((el) => el.color);
              ...
              //retourne true ou false celon si la color du produit à ajouter est déja présente ou non dans le tableau d'objet commande
              let booleanDoublonColor = listColors.includes(newColor);

              //si l'id et la color du produit à ajouter sont déja prensent dans le tableau d'objet commande, ajouter seulement la nouvelle quantité au produit déja présent
              if (booleanDoublonId && booleanDoublonColor) {
                newQuantity = [
                  newQuantity,
                  commande[indexDoublonColor].quantity,
                ];
                ...
                commande.splice(indexDoublonColor, 1, newProduct);
                //envoi "commande" dans le local storage au format json
                localStorage.setItem("product", JSON.stringify(commande));
              }
              //sinon ajoute les valeur de "newProduct" à "commande" et envoi "commande" dans le local storage au format json
              else {
                commande.push(newProduct);
                localStorage.setItem("product", JSON.stringify(commande));
              }
            }

            //sinon créer un tableau, ajoute les valeur de "newProduct" à "commande" et envoi "commande" dans le local storage au format json
            else {
              commande = [];
              commande.push(newProduct);
              localStorage.setItem("product", JSON.stringify(commande));
